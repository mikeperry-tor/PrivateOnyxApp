...
TEEP_DEFAULT_REF := 16650b860ab88e2fa138a11f776a1679b6fe28a8
...
# Public model fixtures can set this without contacting a daemon. A failed
# discovery retains the restrictive model; startup rejects unknown selection.
ifeq ($(PODMAN_SELECTED),true)
PRIVATE_ONYX_DOCKER_GATEWAY_MODE := podman
else ifneq ($(strip $(filter test check check-upgrade,$(MAKECMDGOALS))),)
PRIVATE_ONYX_DOCKER_GATEWAY_MODE ?= isolated
else
PRIVATE_ONYX_DOCKER_GATEWAY_MODE ?= $(strip $(shell python3 podman/startup_health.py docker-gateway-mode --container-bin "$(CONTAINER_BIN)" 2>/dev/null))
endif
PRIVATE_ONYX_DOCKER_GATEWAY_MODE := $(PRIVATE_ONYX_DOCKER_GATEWAY_MODE)
ifeq ($(strip $(PRIVATE_ONYX_DOCKER_GATEWAY_MODE)),)
PRIVATE_ONYX_DOCKER_GATEWAY_MODE := unknown
endif
DOCKER_NETWORK_SUFFIX :=
ifeq ($(PODMAN_SELECTED),false)
DOCKER_NETWORK_SUFFIX := :$(COMPOSE_OVERLAY_DIR)/docker-compose.docker-controller.yml
ifneq ($(PRIVATE_ONYX_DOCKER_GATEWAY_MODE),ordinary)
DOCKER_NETWORK_SUFFIX := $(DOCKER_NETWORK_SUFFIX):$(COMPOSE_OVERLAY_DIR)/docker-compose.docker-isolation.yml
endif
endif
...
.PHONY: integration-network-isolation
integration-network-isolation:
	@python3 tests/validate_docker_host_isolation.py --container-bin "$(CONTAINER_BIN)" --image "$(PYTHON_SLIM_IMAGE)"
...
check-container-health-capability:
	...
	python3 podman/startup_health.py check-docker --container-bin "$(CONTAINER_BIN)" $(DOCKER_EXPECTED_MODE_ARG) --expected-gateway-mode "$(PRIVATE_ONYX_DOCKER_GATEWAY_MODE)"; \
