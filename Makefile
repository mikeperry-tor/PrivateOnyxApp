...
PODMAN_MACOS_OVERRIDE_FILE := $(COMPOSE_OVERLAY_DIR)/docker-compose.podman-macos.yml
...
DOCKER_LINUX_FULL_OVERRIDE_FILE := $(COMPOSE_OVERLAY_DIR)/docker-compose.docker-linux-full.yml
DOCKER_ROOTLESS_OVERRIDE_FILE := $(COMPOSE_OVERLAY_DIR)/docker-compose.docker-rootless.yml
DOCKER_ROOTLESS_FULL_OVERRIDE_FILE := $(COMPOSE_OVERLAY_DIR)/docker-compose.docker-rootless-full.yml
DOCKER_ROOTLESS_TEEP_EMBEDDING_OVERRIDE_FILE := $(COMPOSE_OVERLAY_DIR)/docker-compose.docker-rootless-teep-embedding.yml
...
TOR_DOCKER_FILE := $(COMPOSE_OVERLAY_DIR)/docker-compose.tor-docker.yml
TOR_EGRESS_DOCKER_FILE := $(COMPOSE_OVERLAY_DIR)/docker-compose.tor-egress-docker.yml
...
PRIVATE_ONYX_DOCKER_ENGINE_MODE := podman
else ifneq ($(strip $(filter test check check-upgrade,$(MAKECMDGOALS))),)
# Deterministic checks render both engine variants explicitly and must not
# depend on whichever Docker daemon happens to be active on the test host.
PRIVATE_ONYX_DOCKER_ENGINE_MODE ?= rootful
else
PRIVATE_ONYX_DOCKER_ENGINE_MODE ?= $(strip $(shell python3 podman/startup_health.py docker-mode --container-bin "$(CONTAINER_BIN)" 2>/dev/null))
endif
ifeq ($(strip $(PRIVATE_ONYX_DOCKER_ENGINE_MODE)),)
PRIVATE_ONYX_DOCKER_ENGINE_MODE := unknown
endif
DOCKER_ROOTLESS_SELECTED := $(if $(filter rootless,$(PRIVATE_ONYX_DOCKER_ENGINE_MODE)),true,false)
SHARED_DATA_ENGINE := $(if $(filter true,$(PODMAN_SELECTED)),podman,$(if $(filter true,$(DOCKER_ROOTLESS_SELECTED)),docker-rootless,docker-rootful))
DOCKER_EXPECTED_MODE_ARG := $(if $(filter rootful rootless,$(PRIVATE_ONYX_DOCKER_ENGINE_MODE)),--expected-docker-mode "$(PRIVATE_ONYX_DOCKER_ENGINE_MODE)",)
PRIVATE_ONYX_DOCKER_HOST_GATEWAY := host-gateway
ifeq ($(DOCKER_ROOTLESS_SELECTED),true)
...
export PRIVATE_ONYX_DOCKER_HOST_GATEWAY
ifeq ($(PODMAN_SELECTED),true)
...
PODMAN_MACOS_COMPOSE_SUFFIX :=
PODMAN_MACOS_FULL_COMPOSE_SUFFIX :=
DOCKER_LINUX_COMPOSE_SUFFIX :=
DOCKER_LINUX_FULL_COMPOSE_SUFFIX :=
DOCKER_ROOTLESS_COMPOSE_SUFFIX :=
DOCKER_ROOTLESS_FULL_COMPOSE_SUFFIX :=
...
ifeq ($(HOST_OS),Darwin)
PODMAN_MACOS_COMPOSE_SUFFIX :=:$(PODMAN_MACOS_OVERRIDE_FILE)
PODMAN_MACOS_FULL_COMPOSE_SUFFIX :=:$(PODMAN_MACOS_FULL_OVERRIDE_FILE)
endif
else ifeq ($(HOST_OS),Linux)
ifeq ($(DOCKER_ROOTLESS_SELECTED),true)
DOCKER_ROOTLESS_COMPOSE_SUFFIX :=:$(DOCKER_ROOTLESS_OVERRIDE_FILE)
DOCKER_ROOTLESS_FULL_COMPOSE_SUFFIX :=:$(DOCKER_ROOTLESS_FULL_OVERRIDE_FILE)
ifeq ($(ONYX_RAG_EMBEDDING_SHIM_UPSTREAM_URL),http://host.docker.internal:$(HOST_PORT_TEEP)/v1/embeddings)
DOCKER_ROOTLESS_TEEP_EMBEDDING_COMPOSE_SUFFIX :=:$(DOCKER_ROOTLESS_TEEP_EMBEDDING_OVERRIDE_FILE)
endif
else
DOCKER_LINUX_COMPOSE_SUFFIX :=:$(DOCKER_LINUX_OVERRIDE_FILE)
DOCKER_LINUX_FULL_COMPOSE_SUFFIX :=:$(DOCKER_LINUX_FULL_OVERRIDE_FILE)
endif
endif
PRIVATE_ONYX_HOST_UID := $(shell id -u)
PRIVATE_ONYX_HOST_GID := $(shell id -g)
PRIVATE_ONYX_DOCKER_TOR_UID := $(PRIVATE_ONYX_HOST_UID)
PRIVATE_ONYX_DOCKER_TOR_GID := $(PRIVATE_ONYX_HOST_GID)
PRIVATE_ONYX_DOCKER_TOR_RUNTIME_SOURCE := ./docker-data/tor/docker-runtime
ifneq ($(filter Darwin true,$(HOST_OS) $(DOCKER_ROOTLESS_SELECTED)),)
PRIVATE_ONYX_DOCKER_TOR_UID := 0
PRIVATE_ONYX_DOCKER_TOR_GID := 0
endif
ifeq ($(HOST_OS),Darwin)
PRIVATE_ONYX_DOCKER_TOR_RUNTIME_SOURCE := tor-runtime
endif
export PRIVATE_ONYX_HOST_UID
export PRIVATE_ONYX_HOST_GID
export PRIVATE_ONYX_DOCKER_TOR_UID
export PRIVATE_ONYX_DOCKER_TOR_GID
export PRIVATE_ONYX_DOCKER_TOR_RUNTIME_SOURCE
...
TOR_SUFFIX := $(TOR_COMMON_SUFFIX)$(TOR_EGRESS_SUFFIX)$(TOR_ONION_SUFFIX)$(TOR_PODMAN_SUFFIX)$(TOR_ONION_PODMAN_SUFFIX)$(TOR_DOCKER_SUFFIX)$(TOR_EGRESS_DOCKER_SUFFIX)
...
ONYX_TOKENIZER_CACHE_FILE := docker-data/model-cache/private-onyx/nomic-embed-text-v1/tokenizer.json
...
OPENSEARCH_VALIDATION_MEMLOCK := $(if $(filter true,$(DOCKER_ROOTLESS_SELECTED)),8388608:8388608,-1:-1)
...
LITE_FILES := $(WRAPPER_FILE):$(LITE_OVERRIDE_FILE)$(PODMAN_COMPOSE_SUFFIX)$(PODMAN_MACOS_COMPOSE_SUFFIX)$(DOCKER_LINUX_COMPOSE_SUFFIX)$(DOCKER_ROOTLESS_COMPOSE_SUFFIX)$(TEEP_VPN_SUFFIX)$(TAILSCALE_VPN_SUFFIX)$(CODE_INTERPRETER_NETWORK_SUFFIX)$(TOR_SUFFIX)
FULL_FILES := $(WRAPPER_FILE):$(FULL_OVERRIDE_FILE)$(PODMAN_COMPOSE_SUFFIX)$(PODMAN_MACOS_COMPOSE_SUFFIX)$(PODMAN_FULL_COMPOSE_SUFFIX)$(PODMAN_MACOS_FULL_COMPOSE_SUFFIX)$(DOCKER_LINUX_COMPOSE_SUFFIX)$(DOCKER_LINUX_FULL_COMPOSE_SUFFIX)$(DOCKER_ROOTLESS_COMPOSE_SUFFIX)$(DOCKER_ROOTLESS_FULL_COMPOSE_SUFFIX)$(DOCKER_ROOTLESS_TEEP_EMBEDDING_COMPOSE_SUFFIX)$(TEEP_VPN_SUFFIX)$(TAILSCALE_VPN_SUFFIX)$(CODE_INTERPRETER_NETWORK_SUFFIX)$(TOR_SUFFIX)
LITE_DOWN_FILES := $(WRAPPER_FILE):$(LITE_OVERRIDE_FILE)$(PODMAN_COMPOSE_SUFFIX)$(PODMAN_MACOS_COMPOSE_SUFFIX)$(DOCKER_LINUX_COMPOSE_SUFFIX)$(DOCKER_ROOTLESS_COMPOSE_SUFFIX)$(TEEP_VPN_SUFFIX)$(TAILSCALE_VPN_SUFFIX)$(CODE_INTERPRETER_NETWORK_SUFFIX)$(TOR_DOWN_SUFFIX)
FULL_DOWN_FILES := $(WRAPPER_FILE):$(FULL_OVERRIDE_FILE)$(PODMAN_COMPOSE_SUFFIX)$(PODMAN_MACOS_COMPOSE_SUFFIX)$(PODMAN_FULL_COMPOSE_SUFFIX)$(PODMAN_MACOS_FULL_COMPOSE_SUFFIX)$(DOCKER_LINUX_COMPOSE_SUFFIX)$(DOCKER_LINUX_FULL_COMPOSE_SUFFIX)$(DOCKER_ROOTLESS_COMPOSE_SUFFIX)$(DOCKER_ROOTLESS_FULL_COMPOSE_SUFFIX)$(DOCKER_ROOTLESS_TEEP_EMBEDDING_COMPOSE_SUFFIX)$(TEEP_VPN_SUFFIX)$(TAILSCALE_VPN_SUFFIX)$(CODE_INTERPRETER_NETWORK_SUFFIX)$(TOR_DOWN_SUFFIX)
...
check: test
	...
	python3 -m compileall -q \
		...
		onyx/bootstrap_tokenizer_cache.py \
...
prepare-onyx-tokenizer: onyx-image-ready prepare-full-host-data
	@python3 onyx/bootstrap_tokenizer_cache.py \
		--container-bin "$(CONTAINER_BIN)" \
		--image "$(ONYX_BACKEND_IMAGE)" \
		--output "$(ONYX_TOKENIZER_CACHE_FILE)"
...
check-container-health-capability:
	...
	python3 podman/startup_health.py check-docker --container-bin "$(CONTAINER_BIN)" $(DOCKER_EXPECTED_MODE_ARG); \
	if [ "$(DOCKER_ROOTLESS_SELECTED)" = "true" ] && { [ -z "$(DOCKER_SOCK_PATH)" ] || [ ! -S "$(DOCKER_SOCK_PATH)" ]; }; then \
		echo "ERROR: rootless Docker socket is unavailable at $(DOCKER_SOCK_PATH)" >&2; \
		echo "Start the selected rootless Docker service and select its local context." >&2; \
		exit 1; \
	fi; \
...
ifeq ($(PODMAN_SELECTED),true)
	@COMPOSE_FILE=$(PODMAN_START_FILES) python3 podman/startup_health.py initialize-opensearch \
		--container-bin "$(CONTAINER_BIN)" \
		--opensearch docker-data/opensearch \
		--audit-config onyx/opensearch/audit.yml \
