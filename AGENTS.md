...
- Documentation:
  ...
  - Describe lasting facts directly and replace obsolete text in place. Do not
    recount the work that established them or retain superseded accounts.
    Running a check or discovering an explanation does not, by itself, justify
    a documentation update.
  - Report completed validation results and investigation evidence in the
    conversation or PR description. Active plans may retain consolidated
    acceptance evidence until review, without progress journals.
  - Keep active and deferred plans accurate about intended design, status, and
    accepted decisions. Plans are review artifacts: before implementation is
    complete, document current behavior in the owning `docs/` document and
    link to it from the plan instead of duplicating the lasting specification.
  - Files under `docs/plans/implemented/` are frozen accepted-design records.
    Preserve them after review/merge; document later behavior changes in the
    canonical owning document instead.
- Patch upgrades:
