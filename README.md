...
> **NOTE** The proxy setup has not been audited for leaks. You still likely want the Myst VPN, or at least a host VPN, in case of proxy bypass. In particular, Obscura/Chrome does not support SOCKS5 usernames and passwords, and will bypass the proxy entirely if these are set. For SOCKS proxies, code-interpreter executor pods receive `HTTP_PROXY`/`HTTPS_PROXY` pointing at the local prefetch-blocking-proxy HTTP adapter so `urllib` can use HTTP(S) egress, but proxy env vars are still routing hints rather than a security boundary for arbitrary generated code.
