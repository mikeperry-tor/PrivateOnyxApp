...
Users can optionally enable [Tor](https://www.torproject.org/) for the Agent's internet access. Tor onion service access is supported in the Tor egress mode. Alternatively, [Mysterium VPN](https://github.com/mysteriumnetwork/node) can be used for this purpose, or an upstream proxy, or both. Browsing and network-enabled code use your selected connection settings. See [Privacy and Security](#privacy-and-security-of-this-stack) for protections and limitations.
...
- **Docker-host integration ports and optional LAN access**:
  ...
  - These permissions apply to configured integrations; they do not grant browsing or generated code access to your computer or LAN. See [Privacy and Security](#privacy-and-security-of-this-stack) for platform limitations.
...
Generated code has no network access by default. To let Python and coding tools
fetch public information and download files, set:
...
Web browsing and network-enabled code use your selected connection settings,
and their requests are restricted to public destinations. Access to services
on your computer or LAN requires separately configured integrations.
Unencrypted public HTTP URLs are blocked by default; agents may access HTTPS
URLs and, with `TOR_EGRESS_ENABLED=true`, Tor onion services.

These protections have platform limits. Docker versions below 28 and Podman
may leave services on your computer or its container VM accessible to
compromised software or malicious generated code. Docker 28+ provides stronger
protection after the stack is stopped and started, but complete containment
has not been verified across all supported platforms. See the
