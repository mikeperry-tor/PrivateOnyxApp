...
Docker Compose wrapper for running [Onyx](https://github.com/onyx-dot-app/onyx) with the [teep](https://github.com/13rac1/teep) private verified LLM inference proxy and [SearXNG](https://github.com/searxng/searxng). Search traffic is fetched through [Obscura](https://github.com/h4ckf0r0day/obscura) via CRW, while `open_url` traffic goes through CRW and uses Obscura when CRW escalates from its initial HTTP prefetch. Browser/search traffic normally egresses over a [Mysterium](https://github.com/mysteriumnetwork/node) VPN connection. [Tailscale Funnel](https://tailscale.com/docs/features/tailscale-funnel) integration allows you to access the instance remotely.
...
Edit `.env.wrapper` as needed, based on the `.env.wrapper.example` template. Keep normal local configuration there: ports, provider keys, routing switches, VPN settings, proxy settings, and optional feature knobs.

Do not copy image tags into `.env.wrapper` for routine use. The committed source of truth for stack image tags and source refs is [`stack.versions.env`](stack.versions.env). The Makefile loads `stack.versions.env` first and `.env.wrapper` second, so `.env.wrapper` can still temporarily override a pin while testing an upgrade.

Local stack auth material is generated by the Makefile on each stack start:
`SEARXNG_SECRET`, `USER_AUTH_SECRET`, `CRW_ONYX_API_KEY`, and the MinIO/S3
credentials are not configured in `.env.wrapper`. Restarting the stack invalidates
existing Onyx auth cookies and transient auth tokens, but persisted app data and
MinIO objects do not depend on those values.
