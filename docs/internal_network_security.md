...
not traverse an external upstream proxy.
...
rejected unless `EGRESS_ALLOW_HTTP_URLS=true`. CONNECT on another allowed port
is an opaque TCP stream and is not application-protocol inspected.
