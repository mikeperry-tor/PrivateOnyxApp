...
- Stock engine modules for `google`, `brave`, `duckduckgo`, and `startpage`
...
- `_crw.extract_crw_html()` maps CRW anti-bot failures back to SearXNG engine
  ...
  empty result list.
- `google2`, `brave2`, `duckduckgo2`, and `startpage2` parse the rendered SERP
  DOM with XPath. They replace the stock search-engine variants that are
  blocked or challenge-prone on VPN/datacenter exit IPs.
