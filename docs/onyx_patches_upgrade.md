...
- Patches Onyx's document update gate so wrapper unchanged sentinels are skipped
  even on forced/targeted reindex paths that disable the normal timestamp gate.
...
- `doc-drop-web` and `host-doc-drop-web-proxy` expose a local read-only docs
  directory for the Onyx Web connector.
