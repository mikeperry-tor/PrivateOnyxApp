...
7. Refresh `reference_repos/searxng` to the target SearXNG commit/tag and
   re-check the wrapper SearXNG engine files, `settings.yml` drift, and custom
   engine DOM selectors described in
...
| `searxng/engines/*.py`, `searxng/core-config/*`, `searxng/searxng-proxy-entrypoint.sh` | `searxng-core` | Mounted by `searxng/docker-compose.yml` and optional `docker-compose.proxy.yml` | `reference_repos/searxng/searx/settings_loader.py`, `reference_repos/searxng/searx/settings.yml`, `reference_repos/searxng/searx/limiter.toml`, stock engine modules |
...
- `reference_repos/searxng/searx/limiter.toml`
...
### SearXNG config drift and overlay option

Current findings against `reference_repos/searxng` at `f8ffbf36f903`:

- `searxng/core-config/limiter.toml` is byte-for-byte identical to
  `reference_repos/searxng/searx/limiter.toml`; there are no local limiter
  changes today.
- `searxng/core-config/settings.yml` is a copied full default with a small
  local delta and substantial incidental drift from an older SearXNG default.
  The clear wrapper deltas are `search.formats: [html, json]`,
  `server.secret_key`, `outgoing.request_timeout: 6.0`, the four custom
  CRW-backed engines, and inactive stock Google/Brave/DuckDuckGo/Startpage
  variants.
- The current full copy also differs from upstream in unrelated engine defaults
  such as `qwant`, `yahoo news`, `aol`, `karmasearch`, and older removed/newly
  added engine entries. Treat those as upgrade-review questions, not obviously
  intentional wrapper behavior.

SearXNG supports a cleaner overlay in `settings.yml`:

- `use_default_settings: true` merges the user file onto the image's
  `searx/settings.yml`.
- Mapping values are deep-merged.
- Engine entries are merged by `name`; unknown names are appended, so the
  `google2`/`brave2`/`duckduckgo2`/`startpage2` entries can live in a compact
  overlay.
- `use_default_settings.engines.remove` and `.keep_only` can prune stock
  engines before local engine entries are merged.

Recommended direction for easier upgrades:

- Convert `settings.yml` to a compact overlay after deciding whether to track
  upstream's default enabled-engine set or freeze the current enabled-engine
  set.
- If tracking upstream defaults is acceptable, use `use_default_settings: true`,
  add the custom engine entries, and mark stock Google/Brave/DuckDuckGo/
  Startpage variants inactive or remove them.
- If stable behavior matters more, use
  `use_default_settings.engines.keep_only` with the approved enabled engine
  names plus the four custom engines. That prevents future upstream defaults
  from silently enabling new external engines, but the keep list becomes an
  explicit upgrade artifact.
- Keep `limiter.toml` only if operator-visible warnings from a missing
  `/etc/searxng/limiter.toml` are undesirable. SearXNG's limiter code loads the
  image's built-in `searx/limiter.toml` as the schema/default and only overlays
  `/etc/searxng/limiter.toml` when present.
- Re-check `searxng/searxng-proxy-entrypoint.sh` if the overlay becomes very
  small. Its proxy merge currently finds the top-level `outgoing:` and
  `engines:` blocks and injects `network: direct` into the custom engine
  entries so loopback CRW requests bypass `PROXY_URL`.
...
```sh
diff -u reference_repos/searxng/searx/limiter.toml searxng/core-config/limiter.toml
diff -u reference_repos/searxng/searx/settings.yml searxng/core-config/settings.yml
